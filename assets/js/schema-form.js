  ...
  var builders = {
    ...
    ngModelOptions: function(args) {
      if (args.form.ngModelOptions && Object.keys(args.form.ngModelOptions).length > 0) {
          ...
          var formElement = args.fieldFrag.firstChild.childNodes;

          var inputElement = args.fieldFrag.firstChild.getElementsByTagName("input")[0];
          inputElement.setAttribute('ng-focus', 'inFocus()')
          inputElement.setAttribute('ng-blur', 'blurred()')
      }
    },
        ...
        link: {
          ...
          post: function(scope, element, attrs, sfSchema) {
            ...
            scope.inFocus = function() {
                sfSchema.focusFunction(scope.form.key[0]);
            }

            scope.blurred = function() {
                sfSchema.blurFunction(scope.form.key[0]);
            }
            scope.showTitle = function() {
              return scope.form && scope.form.notitle !== true && scope.form.title;
            };
  ...
  function($compile, $http, $templateCache, $q, schemaForm,  schemaFormDecorators, sfSelect, sfPath, sfBuilder) {
    ...
    return {
      ...
      controller: ['$scope', function($scope) {
        ...
        this.blurFunction = function(key) {
            $scope.blur($scope.stageId, key);
        }
        this.focusFunction = function(key) {
            $scope.focus($scope.stageId, key);
        }
